...
P141 consumption. P145 adds the first tiny local-files-only real-teacher
fingerprint capture path. P146 links that artifact producer to the real
registered student training path through `fingerprint_corridor`, verifies
optimizer steps move parameters, records finite diagnostics, and writes
...
as init-only reference and avoiding any general quality claim. P149 closes the
...
P153 turns Cycle 1 into a measured trajectory with step-0 and cadence-based
...
minimum-sufficient-force selection with lower-rank tie breaking. One-seed runs
...
released mini textbook handoff. P117.1 made confirmed real mode execute actual
...
backend. P142 adds an input-conditioned tiny rehearsal over that same mode,
verifying distinct input IDs produce distinct logits and optimizer steps move
student parameters. P143 adds teacher-side capture config, dynamic
`stat_bands_v0` mode discovery, configurable exemplar reservoir selection,
...
selection still supported. P145 sends tiny HF causal-LM logits through that
calibrated path and records real teacher metadata and consumer sanity. P146
...
it is still a tiny smoke, not a method-vs-method proof. P149 records the arc
findings, open gaps, blocked claims, and constrained next steps.
