...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="P64 compare RADLADS/QRWKV WKV composite balance hook traces."
    )
    ...
    radlads_entries = load_composite_hook_jsonl(args.radlads_trace)
    qrwkv_entries = load_composite_hook_jsonl(args.qrwkv_trace)
    report = compare_composite_hook_traces(
        radlads_entries,
        qrwkv_entries,
        atol=args.atol,
        rtol=args.rtol,
    ...
    print(f"wrote P64 WKV composite balance hook comparison to {args.out}")
