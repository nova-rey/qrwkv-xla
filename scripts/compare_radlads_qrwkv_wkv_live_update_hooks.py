...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="P63 compare RADLADS/QRWKV WKV live update hook traces."
    )
    ...
    radlads_entries = load_live_update_hook_jsonl(args.radlads_trace)
    qrwkv_entries = load_live_update_hook_jsonl(args.qrwkv_trace)
    report = compare_live_update_hook_traces(
        radlads_entries,
        qrwkv_entries,
        atol=args.atol,
        rtol=args.rtol,
    ...
    print(f"wrote P63 WKV live update hook comparison to {args.out}")
