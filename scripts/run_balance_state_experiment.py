...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="P65 compare off vs experimental RADLADS balance-state mode."
    )
    ...
    report = run_balance_state_experiment(
        out_dir=args.out_dir,
        fixture_manifest=args.fixture_manifest,
        seed=args.seed,
        overwrite=args.overwrite,
    )
    print(f"wrote P65 balance-state experiment to {args.out_dir}")
    print(f"status={report['status']}")
    print(f"default_behavior_preserved={report['default_behavior_preserved']}")
    return 0
