...
def test_run_distill_stage_cli_accepts_schedule_config(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            ...
            str(SCRIPT),
            "--config",
            str(ROOT / "configs" / "distill_stage0_adamw_schedule_stub.yaml"),
            "--targets",
            str(bundle_dir),
    ...
    assert "lr_schedule: warmup_cosine" in result.stdout
    assert "final_learning_rate:" in result.stdout


def test_run_distill_stage_cli_accepts_schedule_flags(tmp_path: Path) -> None:
    bundle_dir = _fake_logits_bundle(tmp_path)
    result = subprocess.run(
        [
            sys.executable,
            str(SCRIPT),
            "--config",
            str(ROOT / "configs" / "distill_stage0_logits_stub.yaml"),
            "--targets",
            str(bundle_dir),
            "--student-architecture",
            "tiny_student",
